...
local marriagesound = audio.loadSound( "audio/marriage.wav" )
...
dayTime = function()
  ...
  discussLynch = function()
    ...
    local function clearDiscussLynch()
      ...
      for i=1, table.getn( bridenames ) do
        bridenames[i]:removeSelf()
        bridenames[i] = nil
      end
    ...
    clearMarriageSignal = function()
      print( "clearmarriageSignal" )
      clearDiscussLynch()
      ui.forward:removeEventListener( "tap", clearVoteLynch )
      ui.forward.alpha = 0
      ui.forwardtext.alpha = 0
    ...
    for i=1, settings.setup.players do
      if playerinfo[i].alive == 1 then
        ...
        if i == brideid and playerinfo[i].married == 0 then
          bridenames[1] = display.newImageRect( uiGroup, "images/interface/marry.png", display.contentWidth * 0.075, display.contentWidth * 0.075 )
          bridenames[1].x, bridenames[1].y = display.contentWidth * 0.9, discussalivenames[w].y
          bridenames[1]:addEventListener( "tap", clearMarriageSignal )
        end
  ...
  marriageSignal = function()

    print( "marriageSignal" )

    local function backMarriageDiscussLynch()
      print( "backmarriageDiscussLynch" )
      ui.forward:removeEventListener( "tap", backMarriageDiscussLynch )
      ui.roleactionicon:removeSelf()
      ui.roleactionicon = nil
      stoppedtime = false
      discussLynch()
    ...
    local function proposeMarriage()

      print( "proposeMarriage" )

      local marriageicons = {}
      local marriagenames = {}

      local function clearMarriage( event )

        print( "clearMarriage" )

        for i=1, table.getn( marriageicons ) do
          marriageicons[i]:removeSelf()
          marriageicons[i] = nil
        end
        for i=1, table.getn( marriagenames ) do
          marriagenames[i]:removeSelf()
          marriagenames[i] = nil
        end

        local married = event.target.id

        ui.helper.text = playerinfo[brideid].name .. " ha sposato " .. playerinfo[married].name .. ", che ha il Ruolo di " .. playerinfo[married].role .. ". Congratulazioni!"
        playerinfo[brideid].married = 1
        playerinfo[married].married = 1
        playerinfo[married].revealed = 1

        audio.play( marriagesound )
        ui.roleactionicon = display.newImageRect( uiGroup, "images/roles/" .. playerinfo[married].role .. ".png", display.contentWidth * 0.28, display.contentWidth * 0.28 )
        ui.roleactionicon.x, ui.roleactionicon.y = display.contentWidth * 0.5, display.contentHeight * 0.6

        ui.forward.alpha = 1
        ui.forwardtext.alpha = 1
        ui.forwardtext.text = "Discussione"

        ui.forward:addEventListener( "tap", backMarriageDiscussLynch )

      end

      ui.helper.text = "POSSIBILI SPOSI:"
      ui.rolehandbook.text = ""
      local j = 1
      for i=1, settings.setup.players do
        if playerinfo[i].alive == 1 and brideid ~= i then
          marriageicons[j] = display.newImageRect( uiGroup, "images/interface/marry.png", display.contentWidth * 0.1, display.contentWidth * 0.1 )
          marriageicons[j].x, marriageicons[j].y = display.contentWidth * 0.74, display.contentHeight * 0.34 + (j-1) * display.contentHeight * 0.09
          marriageicons[j].id = i
          marriageicons[j]:addEventListener( "tap", clearMarriage )
          marriagenames[j] = display.newText( uiGroup, settings.playernames[i], display.contentWidth * 0.1, marriageicons[j].y, "GosmickSans.ttf", 18 )
          marriagenames[j].anchorX = 0
          j = j + 1
        end
      end
    end
...
nightTime = function()
  ...
  local function brideReveal()

    print( "brideReveal" )

    local function clearBrideReveal()
      ui.helper.text = ""
      ui.rolehandbook.text = ""
      ui.forward:removeEventListener( "tap", clearBrideReveal )
      endNightTime()
    end

    if playerinfo[brideid].revealed == 0 then

      playerinfo[brideid].revealed = 1

      ui.helper.text = "La Sposa si rivela."
      ui.rolehandbook.text = playerinfo[brideid].name .. " sta cercando disperatamente qualcuno con cui convolare a nozze. Può decidere di dichiararsi ad un Giocatore a sua scelta, in qualsiasi momento del Giorno, sposandolo e rivelandone il Ruolo."

      ui.forward.alpha = 1
      ui.forwardtext.alpha = 1
      ui.forward:addEventListener( "tap", clearBrideReveal )
...
local function computePlayerinfo()
  ...
  for i=1, settings.setup.players do
    ...
    elseif settings.playerroles[i] == "Armaiolo" then
      playerinfo[i].alignment = 0
      gunsmithid = i
    elseif settings.playerroles[i] == "Sposa" then
      playerinfo[i].alignment = 0
      brideid = i
    end
